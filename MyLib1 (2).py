...
Cnx = mysql.connector.connect(
  host="localhost", user="surendra" #,password=""
)
...
def IssueABook():
    ...
    bookresult = Cursor.fetchall()
    if (Cursor.rowcount <= 0):
        print("Sorry! Book having name/author as '%s' not available." % bookSought)
        return
    print("List of books having the name/author as '%s' is : " % bookSought)
    PrintBooks(bookresult)
    #
    # Cheack whether book sought is issued or not. 
    nocopies = int(0)
    studentIssuedTo = ''
